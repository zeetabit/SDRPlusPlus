...
| 25 | Gap ambiguity inside the beam | High | Medium | Medium | **Confounded — redirected upstream (§38–39).** The §17 gap misclassification is not ambiguity: feeding the classifier the TRUE dit removes 56–69% of it (§38), so it is driven by the +38.9% dit **overestimate**, not gaps. §38b traced that to V1 Kalman's ungated dah-absorption runaway; the symmetric fix is `+kalman2` (dit +38.9%→−6.7%). The §40 paired n=96 recheck overturned "blocked only by noise-4.0" (that was a variance artifact of the n=24 survey, ns at t=1.21) — kalman2's real block is a **significant fast-CW regression** (noise2.0-30wpm t=4.79, handkeyed-30 t=2.57), the same slow-vs-fast axis that reverted LR+log (§25). The aimed asymmetric `+ditguard` is **refuted (§39)** — the estimator is bistable under heavy noise, so a one-directional guard collapses ditEst (−57%) and makes noise-3.0/4.0 garbage (CER>1.0). Soft beam-branching survives only for the ~3.5% residual after a dit fix, not as a first move |
...
| 31 | WPM-locked adaptive BPF (runtime tap update) | N/A | High | Medium | **Built, real-audio-validated, not promoted — variance blocker (§29–35).** `legacy+bpfauto`: input-referred (pre-BPF) SNR trigger + convergence gate + out-of-band guard + garbage floor (§33). Real audio: noiseAmp 1.0 CER 0.121 → 0.026 (~5×, t=−16); synthetic 6 wins, 0 significant regressions, byte-identical on clean/hand-keyed/qrm/qrn. Acquisition gap refuted (§34, blocked by the garbage floor). **Promotion blocked (§35):** better mean/p95/max on worstcase but worse on 16/96 seeds — narrowing adds QSB-dip variance, fails single-seed MSG_CQ gates (0.957). A maintainer judgment call (better-mean-worse-tail), not a re-ratchetable gate. Stays a variant; `legacy` default. Full-close needs raw-IQ re-filtering (front end not re-runnable) |
...
| 39 | Regime timing selector (`legacy+select`) | High | High | High | **Campaign's strongest candidate (§44–48).** Headroom measured first (§44 trellis not worth it; §47 conservative-gate ceiling +0.17 on good-SNR hand-keyed). Runs kalman2s + log in parallel, routes to log on jittered good-SNR signals (hand-keyed, where log wins §46) and kalman2s otherwise, gated on dit-CV AND getSNR (`ITiming::setSnr` plumbing) so weak hand-keyed stays on kalman2s (§46b). **Paired n=384: 13 better / 0 significant worse** — all hand-keyed 15–40 BETTER (the win kalman2s couldn't get), worstcase t=−30, every noise win retained, and kalman2s's significant noise3.0-25wpm regression eliminated (select strictly dominates kalman2s). 6 HARM all ns; 2 clear at n=768, 4 residual are garbage cells (CER 0.89–0.97) + one moderate-noise hand-keyed, none significant. Promotion = maintainer judgment on 4 ns bounds, stronger case than kalman2s. Benchmark params grounded in real VFO (§45, REF_BW_VFO=3000); gate gains weak-hand-keyed cells. **§49 re-armable latch**: model-switching corrupts borderline signals via dit-discontinuity; fixed by freezing the mode once settled and re-opening only on a noise-robust dit-drift trigger at word gaps (operator/WPM change) — fixes contest instability + the latch smell, operator-change validated. Promotion still blocked: moderate-noise (0.0→0.0012) unpassable by any non-V1 timing, contest (0.008→0.0125) inherited from kalman2s. Ships as strongest/cleanest/operator-robust variant |
| 38 | Speed-switched V2 (`legacy+kalman2s`) | High | High | Medium | **Strongest dit-source variant, superseded by #39 (§41–43).** V2's fix wins slow/moderate but regresses fast CW (§40); switch the whole V1/V2 behaviour on a smoothed, hysteresis WPM (band [26.5,29.5]) — decomposition proved the regression is V2's dah-gain, not the gate, so the package must switch together. Eliminates both significant fast-CW regressions and dominates the **usable regime** (noise ≤2.0 all speeds better/ns, worstcase t=−16). §42: the two n=96 HARM (handkeyed-25, noise4.0) were pure power — clear at n=192/768 with genuinely-better means. **But §43 closed a gate coverage gap** (no fast×heavy cells) that hid the truth: on the full 25/30wpm × noise2/3/4 grid, kalman2s regresses the fast×heavy **garbage cells** (noise3.0-25wpm t=2.33 WORSE; both CER>0.9). Latch-at-lock (§43b) refuted — traps 30wpm-n3/n4 in V2 (3 regressions vs 1). The switch keys on the speed estimate heavy noise corrupts — the §33 wall, now bounding even a speed-adaptive dit fix. Complete gate: 9 better / 1 worse / 6 harmful, all failures in garbage cells. `legacy` stays default; kalman2s ships as a variant. **Gate permanently gains the fast×heavy grid** |
| 37 | Dit-bias confound + source + asymmetric guard | High | High | Medium | **Confound + measured negative (§38–40).** `[gap-noise-confound]` / `[gap-dit-source]` / `[ditguard]` / `[kalman2-recheck]` + `setDitOverride`/`TIMING_KALMAN_GUARD`. Proved the #25 gap error is upstream dit bias (§38), traced it to V1's one-directional dah-absorption runaway (§38b), refuted the aimed asymmetric guard (§39, bistable collapse → CER>1.0 at noise-3.0), and rechecked `+kalman2` at paired n=96 (§40): its noise-4.0 block is a variance artifact (ns, t=1.21), the real block is a significant fast-CW regression (noise2.0-30wpm t=4.79) on the same axis as LR+log (§25). The symmetric dit fix trades slow/moderate for fast-CW robustness — a mapped, rejected tradeoff, not an intractable wall |
...
### SparkGap-derived line (2026-07-22, §52)

Three levers extracted from the SparkGap skimmer (ITILA: two-state HMM keystate,
EM-fit per window, forward-backward soft posteriors, late quantisation, 16-bin
WPM marginalisation; GPL-3.0, license-compatible). Ordered by **cleanest first
win**, not by gain — the same discipline that ranked §44.

| rank | item | status after §52.1 probes |
|---|---|---|
...
| A | **#41 model-fit quality metric** | **Stage 0/1/2 DONE (§52.3)** — `model_fit.h` scorer built + orthogonal to SNR (22 dB het → 0.0004 vs 12 dB CW 0.29); validated on the REAL front-end envelope (CW 2.51 vs carrier/noise ~0 — the BPF sharpens it); wired into `Channel.modelFit` (passive, throttled, decode byte-identical, suite 1679/228). Next: Stage 3 — use it in `channel_manager` spawn/prune/merge behind a config flag + `getMarkers`, on the `test_channel_manager` harness |
...
| C | **#42 soft-posterior detector** | **The big bet — deferred to a measured headroom check.** The only lever that attacks the noise wall (`detectHR +0.28`), Very-High complexity (joint core). Do not start until #41 pays down the likelihood machinery and a probe shows a real detectHR fraction is capturable. Supersedes #24/#27 |
