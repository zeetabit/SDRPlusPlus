...
| C | **#42 soft-posterior detector** | **Headroom check DONE (§52.5) — JUSTIFIED, strongest build case in the campaign.** `[softdet-headroom]`: a perfect DETECTOR (real Kalman timing) drives noise2/3/4 to CER **0.0000** — the entire noise wall is a detector failure, not timing/decoder. Ceiling +0.82/+0.91 (noise3/4), the largest headroom measured anywhere. Existing soft detectors (lr/lr+soft) capture almost none (+0.02–0.06), so a full forward-backward HMM has nearly the whole ceiling as residual — the OPPOSITE of §44's trellis (which ran on destroyed durations). Caveat: oracle uses ground truth, so +0.8 is a CEILING; gate the build on an early achievable-fraction probe (minimal soft detector vs legacy on the noise ladder). Very-High complexity (joint core). Supersedes #24/#27 |
