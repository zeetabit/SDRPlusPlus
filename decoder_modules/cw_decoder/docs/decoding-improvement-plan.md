...
| 31 | WPM-locked adaptive BPF (runtime tap update) | N/A | High | Medium | **Ceiling measured — the campaign's largest lever (§29).** Ground-truth WPM-matched BPF (`[bpf-wpm]`) vs legacy, paired n=96: **7 significant wins, 0 significant regressions.** noise3.0 0.817 → 0.075 (11×, t=−39), and it closes the fast+heavy residual §21–28 could not: noise2.0-25wpm 0.184 → 0.023 (t=−25), -30wpm 0.287 → 0.125 (t=−18). Dissolves §4's speed/noise tradeoff exactly as predicted; fast CW unregressed (matched wide filter). Front-end only, minimal change to the default. **Blocker:** 6 hand-keyed profiles flag non-inferiority (small, ns) — matched-to-white-noise over-narrows a jitter-limited signal; needs a noise-aware bandwidth floor, then the runtime lock |
