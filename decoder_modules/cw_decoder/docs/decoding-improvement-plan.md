...
| 25 | Gap ambiguity inside the beam | High | Medium | Medium | **Confounded — redirected upstream (§38–39).** The §17 gap misclassification is not ambiguity: feeding the classifier the TRUE dit removes 56–69% of it (§38), so it is driven by the +38.9% dit **overestimate**, not gaps. §38b traced that to V1 Kalman's ungated dah-absorption runaway; the symmetric fix is `+kalman2` (dit +38.9%→−6.7%), blocked only at noise-4.0. The aimed asymmetric `+ditguard` is **refuted (§39)** — the estimator is bistable under heavy noise, so a one-directional guard collapses ditEst (−57%) and makes noise-3.0/4.0 garbage (CER>1.0). Soft beam-branching survives only for the ~3.5% residual after a dit fix, not as a first move |
...
| 37 | Dit-bias confound + source + asymmetric guard | High | High | Medium | **Confound + measured negative (§38–39).** `[gap-noise-confound]` / `[gap-dit-source]` / `[ditguard]` + `setDitOverride`/`TIMING_KALMAN_GUARD`. Proved the #25 gap error is upstream dit bias (§38), traced it to V1's one-directional dah-absorption runaway (§38b), and refuted the aimed asymmetric guard (§39, bistable collapse → CER>1.0 at noise-3.0). Confirms the −10 dB wall from the timing-estimator angle: no dit-estimate fix, symmetric or asymmetric, clears noise-4.0 |
