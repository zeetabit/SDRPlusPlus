...
> **PROD-ONLY, never a fix mechanism (user, 2026-07-25).** The unreal-WPM clamp is a
> field backstop and *only* that. It MUST NOT be used to make a test pass or to "fix" a
> decode: clamping the estimate SUPPRESSES the symptom but never RECOVERS the content, so
> leaning on it hides the real acquisition bug. **Evidence (`[unreal-wpm]`, moderate-noise
> seed 0):** `fb+sel` "SO Q…" → `fb+sel+rsg` "5 Q…" (rej=1) — the 100 WPM lock is rejected
> but the first token is still garbage, not "CQ". Across the full ladder the guard changes
> **zero** element errors. So for tests and fixes treat the guard as OFF (or assert its
> counter is zero) and fix the SOURCE — the acquisition that emits the spurious short
> elements — not the clamp. Same discipline as word-correction OFF under test.
...
## 15. B (first-char) — SHIPPED FIXES (2026-07-25, evidence overturned the plan)

Fresh traces (`[acq-axis]` both seeds, `[onset-env]` sparkline) split the first-char artifact
by LAYER, not the single "raw-IQ re-filter" mechanism the DR-5 note assumed:

- **Detector layer** (seed 0 `SO Q`, and the O/MM/TT class): fb acquisition under cold/immature
  θ. The hard `select` decodes seed 0 fine, so it is fb-specific — NOT the BPF (the first char
  is under the WIDE start-filter; the 32 Hz gate fires later).
- **Front-end layer** (seed 5 `TRQ`): the first char's intra-element gaps are FILLED in the
  post-BPF envelope (dip only to ~60 % of peak, never the noise floor) — a genuine onset merge
  no re-decode can recover. This IS the raw-IQ class; **deferred** (low impact, ladder Δ≈0).

**Fix 1 — θ-refit reDetect (cont path).** `FBDetector::reDetect` now WARM-STARTS from matured θ
+ online-EM stats and adapts LOCALLY down the window (mirrors `updateEmission`) instead of one
frozen θ (which, fit on the narrow tail, mis-fits the wide-filter onset and re-merges it).
Recovers the detector class: seed 0 cont `TRQ CQ CQ` → `CQ CQ CQ`. `[cont]` ladder **−746 → −756**
(snr4 +21→+13, no new regressions); cont-only, suite byte-identical.

**Fix 2 — gap-based seed disambiguation (the documented "correct fix", timing.h seed).** The
KNOWN DEFECT `ditEst = min(3 elements)` seeds dit 3× high on an all-DAH opening (O=---, MM). The
`≥2 dit+≥2 dah / <10 % variance` gate §14 proposed == "defer the seed", which the code documents
as ALREADY-TRIED and regressing clean decode 0.01→0.364 (it also defers the lock+retroDecode).
Built instead the intra-character element-gap anchor (≈1 dit) the code's own note points to:
`AdaptiveTiming` feeds the shortest pre-lock gap to `KalmanTiming`/`LogTiming` via
`setGapDitHint`; the seed corrects ONLY on a tight all-one-type run whose `minD/gapDit` is in the
PHYSICAL dah:dit band [2.2, 4.0]. Clean `OM OM DE W1AW`/`MM MM DE W1AW` now decode correctly
(were `DI5SEAW`/`BHHAW`); mixed (CQ) byte-identical; `TT` unchanged (single-element openings have
no intra-char gap → dit genuinely unknowable). **Anti-masking note:** the FIRST attempt used a
loose `minD > 1.8·gapDit` and regressed noise (+2 catastrophic: weak-signal 0.7, benchmark 0.76)
because `min(gap)` is as noise-fragile as `min(element)`; the physical-ratio band rejects
off-ratio noise gaps. Proven byte-identical to baseline by an A/B disable (the 4 multiseed
backlog failures are IDENTICAL fix-on/off → pre-existing, not regressions). Full suite 9-backlog.
This is the SOURCE fix, NOT the unreal-WPM clamp (which is prod-only, §14).

New diagnostics (all `[.]`): `[seed-defect]`, `[onset-env]`, `[acq-axis]` (extended to seeds 0&5).
