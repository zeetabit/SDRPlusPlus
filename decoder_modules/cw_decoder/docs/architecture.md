...
└── tests/
    ...
    ├── cw_parallel.h                 parallel seed execution; CW_TEST_THREADS=1
    │                                 forces serial (results are invariant)
    └── test_*.cpp                    default suite runs in seconds; hidden [.]
                                      probes are excluded from it and cost more
    └── fetch_recordings.sh           populates recordings/ for the [recording] gate
                                      (gitignored; ARRL audio and QST text are not committed)
```
...
| Mark unmeasured claims (⊘ / ⚠ / ✗ / 📎) | Of 23 predictions made from code reading, 13 were wrong. Unmarked plausible claims have roughly even odds. The 22nd: §16.4 predicted noise would degrade gap classification by *fragmenting gaps*. Compounding was confirmed, but the driver is a **39% dit overestimate** — the right chain, the wrong corrupted duration (§17.3.2). The 23rd was correct and is the exception that shows the rule: the orphaned `audioToIQ` was predicted broken from the filter algebra *before* running it, and the prediction named the observable (envelope ≈ 0 for any Ω). Code reading pays when it yields a falsifiable prediction, not a plausible story. |
...
| A test suite can only express defects its **inputs contain** | Period and comma were unmapped for the project's lifetime and the whole suite passed, because `MSG_CQ` and `MSG_FULL` are the only messages sent and neither has punctuation. The gap was in the fixtures, not the assertions — no amount of added checks over the same two strings would have found it. |
| Ground truth the system generated itself cannot falsify that system | Every pre-2026-07-20 number scores the decoder against signals this repo produced, so it confirms agreement with the generator's assumptions. Real ARRL audio with published text found two defects in its first run. |
| A helper broken on the **cold path** reports success on the warm one | `fetch_recordings.sh` staged downloads through `.part` files, which ffmpeg cannot infer a format from. With a populated cache it printed "cached" and exited 0 while being broken for every first-time user. Test the empty-state path explicitly. |
| Two unrelated fixes for the same errors means the errors are **marginal**, not evidence of either mechanism | `legacy+mf` clears the 35 WPM errors, which matched the matched-filter dropout hypothesis exactly — until `legacy+kmeans` and `legacy+median`, which touch no filter, cleared them too (§19.3.2). A confirmed prediction is not a confirmed mechanism. |
| Winning on the data you have does not survive the regime you lack | `legacy+edge+log` is the best core on real audio (mean 0.0013 vs legacy 0.0044) and more than doubles CER on synthetic heavy noise. Every real recording is 19-76 dB SNR, so the sweep cannot speak about the regime that decides promotion (§19.4). |
...
| `tests/test_recording.cpp` | Not in the build; broken audio→IQ; silent pass | **FIXED 2026-07-20 (§18).** Found orphaned (§17.5), then found to be unusable even if built: its `audio(t)·e^{jΩt}` conversion places the signal outside the ±100 Hz filter for any Ω — measured SNR 3.0 against 71.6 for `re`=samples, `im`=0. Rewritten against the ARRL W1AW gate, now in the build, and fails on a missing cache instead of returning early before its only `REQUIRE`. |
| `morse_tree.h` punctuation | Period and comma unmapped | **FIXED 2026-07-20 (§18.3).** Periods decoded as `*` (SK, nearest mapped node), commas vanished. Survived the whole suite because `MSG_CQ` and `MSG_FULL` are its only messages and neither contains punctuation; ARRL reference text carries 23 periods and 21 commas. Now `tree[84]`/`tree[114]`, gated by `[punctuation]` and by the real-recording gate. |
